...
In Advanced LIGO, the test masses are suspended by four-stage pendulums, known as the quad suspensions\cite{Aston_2012}. 
...
distributed between 1 and 4 Hz, providing a passive suppression of seismic motion along 
...
The beam is then handed off to the Input Optics subsystem~\cite{Mueller_2016}, where phase modulation sidebands are applied and the power of the beam is controlled, before the beam is passed to the in-vacuum suspended input mode cleaner cavity. 
...
The main difference in the Advanced Virgo implementation of the Thermal Compensation System is the addition of a scanning CO$_2$ projector, which compared to a fixed-mask one promises greater flexibility and adaptability of the shape of the projected pattern.
...
One of the main advantages to the small beam waist size solution is that thermal deformations of the test masses caused by 
...
The optical layout of Advanced Virgo is very similar to that of Advanced LIGO, although restrictions in available vacuum enclosure space prevent the use of stable recycling cavities. Moreover, no equivalent of the arm length stabilization system was included in the baseline Advanced Virgo design.
...
mirror motions at low frequencies can be of the order of several wavelengths, is that length control loops are essential in order 
...
The alignment of optics must also be sensed and controlled in gravitational wave detectors~\cite{Fritschel1998}.
...
Advanced Virgo uses a similar scheme, although an alternative method developed by Dana Anderson was originally used in Virgo, whereby the sideband frequencies were chosen such that higher-order spatial modes of the sidebands would be co-resonant in some of the optical cavities with the carrier fundamental mode~\cite{Anderson1984}.
